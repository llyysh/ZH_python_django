...
api_user = 'user_yikatong_2'
# api_user = 'user_xinyisou'
# api_user = 'user_yushankeji'
# api_user = 'user_juhekeji'
# api_user = 'user_haichuanshuzi'
# api_user = 'user_zhongdianyun'
...
def get_data_frame(data, date_type):
    ...
    if date_type == UPSTREAM_FLAG:
        ...
        downstream_data_dict['req'].append(data['request']['request'])
        downstream_data_dict['api_user_code'].append(api_user_code)
        downstream_data_dict['interface_code'].append(interface_code)
        downstream_data_dict['msg'].append(data['response']['payload'])
        downstream_data_dict['time'].append(data['executionTime'])
        downstream_data_dict['uuid'].append(data['uuid'])
        ...
        if api_file.endswith('.gz'):
            with gzip.open(file_path + api_file, 'rt',encoding='utf-8') as file:
                for line in file:
                    ...
                    if not is_exception:
                        if line.__contains__(DOWNSTREAM_FLAG):
                            if line_json['chargable'] and line_json['apiUserCode'] == api_user:
                                # print(line_json['response'])
...
downstream_data_frame.columns = ['时间', '入参', '用户', '接口', '结果', '耗时', 'uuid']

out_Path = './bill_data/bill_result/bill_detail_' + api_user + '_' + month + '.xlsx'

writer = pd.ExcelWriter(out_Path, engine='xlsxwriter')

downstream_data_frame.to_excel(writer, sheet_name='客户', index=False)
writer.close()
