...
# api_user_name = 'user_yikatong_2'
# api_user_name = 'user_xinyisou'
# api_user_name = 'user_jiaotongyinhang'
# api_user_name = 'user_yifengkeji'
api_user_name = 'user_shujubao'
...
def get_data_frame(data, date_type):
    ...
    if date_type == UPSTREAM_FLAG:
        ...
        if 'mobile' in data['request']['request']:
            if data['response']['payload']['isp'] is not None:
                ...
                for line in file:
                    ...
                    if not is_exception:
                        if line.__contains__(UPSTREAM_FLAG):
                            ...
                            # if line_json['apiUserCode'] == api_user_name:
                            if line_json['apiUserCode'] == api_user_name and line_json['chargable']:
...
# upstream_data_frame.columns = ['日期', '用户', '接口', '数量', '单价', '上游', '上游接口']
downstream_data_frame.columns = ['日期', '用户', '接口', '运营商', '数量', '单价']

# upstream_result = upstream_data_frame.groupby(['上游', '上游接口', '单价'], as_index=True)['数量'].value_counts().reset_index(
#     name='count')
downstream_result = downstream_data_frame.groupby(['日期', '用户', '接口'], as_index=True)[
    '数量'].value_counts().reset_index(
    name='count')
# print(upstream_result)
print(downstream_result)

# upstream_result.columns = ['上游', '接口', '单价', '数量1', '数量']
# downstream_result.columns = ['用户', '接口', '运营商', '单价', '数量1', '数量']


# out_Path = './bill_data/bill_result/bill_user_select'+api_user_name+'_' + month + '.xlsx'
out_Path = './bill_data/bill_result/bill_'+api_user_name+'_' + month + '.xlsx'
